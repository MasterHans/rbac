 ...
 * roles - тут можно указывать имя "Роль" (admin, author, accountant) , а так же "Разрешение". Роль - может совпадать
   с именем пользователя но это не имя пользователя.
   ### Пример:
   Разрешение:
   ...
   Роль:
      ...
      [
           'actions' => ['test', 'gii', 'sag-debug', 'ajax'],
           'allow' => true,
           'roles' => ['admin'],
       ], 

ACF привязываем к пользователю. Настраивем "matchCallback" чтобы он пропускал пользователя с указанным именем.
